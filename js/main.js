...
// Mobile nav toggle + entrance animations
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (navToggle && navCategories) {
    navToggle.addEventListener("click", () => {
      const isOpen = navCategories.classList.toggle("is-open");
      navToggle.setAttribute("aria-expanded", String(isOpen));
    });

    navCategories.querySelectorAll("a").forEach((a) => {
    ...
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape" && navCategories.classList.contains("is-open")) {
        navCategories.classList.remove("is-open");
        navToggle.setAttribute("aria-expanded", "false");
      }
    });
  ...
  // Smooth entrance animation for real repeating content blocks
  ...
  document
    .querySelectorAll(".domain-card, .candidate-card, .project-card")
    .forEach((el) => {
      el.style.opacity = "0";
